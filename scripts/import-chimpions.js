...
const all=[];let page=1,total=null,guard=0;
while(guard++<100){
  ...
  const body=await r.json();total=Number(body.total??body.count??total??expected);
  ...
  const hasMore=body.hasMore??body.has_more??(items.length>0&&all.length<(total||expected));if(!hasMore)break;page=Number(body.nextPage??body.next_page??page+1)
}
...
const cards=[...byKey.values()].sort((a,b)=>a.id-b.id),target=total||expected;
if(!allowPartial&&cards.length!==target)throw new Error(`Collection import incomplete: ${cards.length}/${target}. Set ALLOW_PARTIAL_IMPORT=1 only for local fallback work.`);
await mkdir('public/data',{recursive:true});await writeFile('public/data/chimpions.json',JSON.stringify({source:base,importedAt:new Date().toISOString(),reportedTotal:target,cards},null,2));
console.log(JSON.stringify({reportedTotal:target,discovered:all.length,valid:cards.length,duplicates:all.length-cards.length},null,2));
