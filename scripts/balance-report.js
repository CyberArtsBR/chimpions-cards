...
const expected=manifest.expectedTotal??manifest.reportedTotal??null;
const report={cards:cards.length,expected,apiReportedTotal:manifest.apiReportedTotal??null,provisional:Boolean(expected&&cards.length!==expected),statBudget:360,bounds:[34,86],distributions,strongestByBestAttribute:strength.slice(0,10),weakestByBestAttribute:strength.slice(-10).reverse(),starterStudy:{classic:starterStudy(MODES.classic),tactical:starterStudy(MODES.tactical)}};
